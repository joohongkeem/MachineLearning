...
action = np.loadtxt('./_19_4_data.txt', delimiter=',')
    # delimiter 는 '구분 문자' 라는 의미
...
def gradient_descent(x,y) :
    ...
    for _ in range(m) :  # 100 번
        ...
        #print('e :', e)
        g = np.dot(x.T, e)                      # Gradient Descent 구함
                                                # (3, 1) = (3, 100) * (100, 1)
                                                # g = e * x --> delta += (hx - y[i])*x[i]
...
def gradient_stochastic_random(x, y) :
    ...
    for i in range(m*10) :
        ...
        z = np.sum(x[p] * w)        # x의 한 행과 w를 곱해 더함 (배열의 곱셈) --> scalar 0차원
        h = sigmoid(z)              # scalar
        e = h - y[p]                # (1, ) = scalar - scalar
        g = x[p] * e                # (3, )
        w -= lr * g                 # (3, ) = (3, )
...
def gradient_minibatch(x,y) :
    ...
    for _ in range(epochs) :
        count = m // batch_size         # 자투리 버림(count = 20), // 연산자는 몫을 계산한다.
        for j in range(count):         # 20번 반복
            n1 = j * batch_size         # 1배치 -> 다음번 배치 -> 다음번 배치
            n2 = n1 + batch_size

            z = np.dot(x[n1:n2], w)     # (5, 3) * (3, 1)   --> (5, 1)
            h = sigmoid(z)              # (5, 1) = sigmonid(5, 1)
            e = h - y[n1:n2]            # (5, 1) = (5, 1) - (5, 1)
            g = np.dot(x[n1:n2].T, e)   # Gradient Descent 구함, (3, 1) = (3, 5) * (5, 1)
            w -= g * lr                 # (3, 1) = (3, 1)
...
def gradient_minibatch_random(x,y) :
    ...
    for i in range(epochs) :
        ...
        for j in range(count):         # 20번 반복
            ...
            e = h - y[n1:n2]            # (5, 1) = (5, 1) - (5, 1)
            g = np.dot(x[n1:n2].T, e)   # Gradient Desncent 구함, (3, 1) = (3, 5) * (5, 1)
            w -= g * lr                 # (3, 1) = (3, 1)


# 하나의 epochs가 끝날때 섞어줌...   -- > 성능을 조금 더 향상 시키려는 시도!! (데이터를 섞음으로써 더 좋은 결과를 얻을 수 있는지 확인!!)
...
def gradient_minibatch_shuffle(action) :
    ...
    for i in range(epochs) :
        ...
        # 하나의 epochs가 끝날때 섞어줌...
        # 단.  요소의 크기가 클 경우 메모리에 과부하가 생길 수 있으므로
        # --> 인덱스 배열을 만들어 인덱스의 셔플을 통해 데이터를 가져옴
...
def show_elapsed(f, x, y) :
    start = time.clock()
    f(x, y)
    print('time of {0} = {1}'.format(f,time.clock() - start))
...
# 데이터의 개수가 엄청 크지않기 때문에, 그래프로 성능을 비교하기에는 무리가 있다!
